...
uint16_t co2_ppm;
float temperature_c, humidity_p, pressure;
int p_cau, p_war;
int elapsed_time = 0;
...
void setup()
{
    ...
    {
        M5.Lcd.println("fail! SCD30");
        Serial.println("Air sensor not detected. Please check wiring. Freezing...");
        while (1)
            ;
    }
    //Initializes sensor for air quality readings from "Example1_BasicReadings.ino" for SGP30
...
void updateDisplay()
{
    ...
    spr_values.printf("Press::%4.1f hPa\n", pressure);
...
//        temperature_c = airSensor.getTemperature();
        temperature_c = bme280Sensor.readTempC();
        Serial.print(" temp(C):");
        Serial.print(temperature_c, 1);

//        humidity_p = airSensor.getHumidity();
        humidity_p = bme280Sensor.readFloatHumidity();
        Serial.print(" humidity(%):");
        Serial.print(humidity_p, 1);

        pressure = bme280Sensor.readFloatPressure() / 100; // Pa -> hPa
